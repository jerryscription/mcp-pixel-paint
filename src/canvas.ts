import { Color, parseColor, PALETTE } from './colors.js';
...
export class PixelCanvas {
  ...
  constructor(width: number, height: number) {
    ...
    this.pixels = Array(height).fill(null).map(() => 
      Array(width).fill(null).map(() => ({ ...PALETTE.transparent }))
    );
  }
  ...
  setPixel(x: number, y: number, color: string | Color): void {
  ...
  clear(color: string | Color = 'transparent'): void {
  ...
  drawRectangle(x: number, y: number, width: number, height: number, color: string | Color, filled: boolean = false): void {
    const plotColor = parseColor(color);
  ...
  floodFill(x: number, y: number, newColor: string | Color): void {
    if (x < 0 || x >= this.width || y < 0 || y >= this.height) {
      return;
    }
  ...
  static loadFromJSON(filename: string): PixelCanvas {
